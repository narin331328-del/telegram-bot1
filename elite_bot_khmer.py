...
# ដាក់ Anthropic API Key របស់អ្នកនៅទីនេះដោយសុវត្ថិភាព
ANTHROPIC_API_KEY = (
    "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"
)
...
def send_welcome(message):
  ...
  bot.reply_to(
      message,
      "សួស្តី លោក/លោកស្រី! ខ្ញុំជា AI (Claude 3.5 Sonnet) ជំនួយការផ្នែក IT"
      " និង Cybersecurity របស់អ្នក។ តើថ្ងៃនេះចង់ឱ្យខ្ញុំជួយអ្វីខ្លះ?",
      reply_markup=markup,
  )
...
def send_status(message):
  bot.reply_to(
      message,
      "ស្ថានភាពប្រព័ន្ធរបស់ Bot (Claude 3.5 Sonnet) គឺកំពុងដំណើរការធម្មតា ១០០%"
      " (Active) និងមានសុវត្ថិភាពល្អប្រសើរ។",
  )
...
def handle_message(message):
  ...
  try:
    if text == "🛡 Ethical Hacking & Security":
      bot.reply_to(
          message,
          "ខ្ញុំត្រៀមខ្លួនរួចជាស្រេចដើម្បីជួយពិភាក្សាជូនលោក/លោកស្រីអំពី Penetration"
          " Testing, OWASP Top 10, Nmap scanning, និង Linux Security។",
      )
    elif text == "🌐 Networking & Cisco":
      bot.reply_to(
          message,
          "ខ្ញុំអាចជួយលោក/លោកស្រីពន្យល់ពី Subnetting, OSI Model, TCP/IP, VLAN"
          " និង Cisco Configurations។",
      )
    elif text == "💻 Programming & Code Debug":
      bot.reply_to(
          message,
          "សូមផ្ញើ Code របស់លោក/លោកស្រីមកទីនេះ ខ្ញុំនឹងជួយ Debug"
          " និងពិនិត្យរក Error ជូន!",
      )
    elif text == "🧠 ចំណេះដឹងបច្ចេកវិទ្យាទូទៅ (គ្រប់វ័យ)":
      bot.reply_to(
          message,
          "មុខងារនេះសម្រាប់រៀបចំចំណេះដឹងទូទៅអំពីបច្ចេកវិទ្យា SmartPhone"
          " អ៊ីនធឺណិត និងសុវត្ថិភាពអនឡាញ។",
      )
    else:
