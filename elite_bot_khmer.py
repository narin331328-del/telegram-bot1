...
- នៅពេលឆ្លើយសំណួរបច្ចេកវិទ្យាទូទៅ (សម្រាប់មនុស្សចាស់ ឬក្មេង) ត្រូវប្រើឧទាហរណ៍ប្រៀបធៀបងាយៗ និងភាសាខ្មែរច្បាស់លាស់(ពួកយើងសុំទោសផងបើការបកប្រែជាភាសាខ្មែរខុសអក្ខរាវិរុទ្ធ)។
- នៅពេលឆ្លើយសំណួរកម្រិតខ្ពស់ផ្នែក IT/Cybersecurity ត្រូវផ្តល់នូវការយល់ដឹងស៊ីជម្រៅផ្នែកបច្ចេកវិទ្យា, គោលគំនិតនៃការវាយប្រហារបែបសីលធម៌ (Ethical Hacking), និងយុទ្ធសាស្ត្រការពារប្រព័ន្ធបច្ចេកវិទ្យា (Defensive Strategies)។
...
def send_welcome(message):
    ...
    item1 = types.KeyboardButton("🛡️ Ethical Hacking & Security")
...
def handle_message(message):
    text = message.text
    try:
        if text == "🛡️ Ethical Hacking & Security":
            ...
            bot.reply_to(message, "ខ្ញុំអាចជួយអ្នកពន្យល់ពី Subnetting, OSI Model, TCP/IP, VLAN, Routing Protocols, និង Cisco Command Line Configurations។")
        elif text == "💻 Programming & Code Debug":
            bot.reply_to(message, "សូមផ្ញើCodeរបស់អ្នកមកទីនេះ (Python, Bash, C++, JavaScript...) ខ្ញុំនឹងជួយ Debug និងពិនិត្យរក Error ជូន!")
        elif text == "🧠 ចំណេះដឹងបច្ចេកវិទ្យាទូទៅ (គ្រប់វ័យ)":
            bot.reply_to(message, "មុខងារនេះគឺសម្រាប់រៀបចំចំណេះដឹងទូទៅអំពីបច្ចេកវិទ្យា SmartPhone អ៊ីនធឺណិត និងសុវត្ថិភាពអនឡាញ ដែលងាយយល់បំផុតសម្រាប់គ្រប់វ័យ (ក្មេងដល់ចាស់)។ តើលោកអ្នកចង់ដឹងពីអ្វី?")
        elif text == "🔍 Scan/Analyze Link or Text":
            bot.reply_to(message, "សូមផ្ញើតំណភ្ជាប់ (Link) ឬ Log មក ខ្ញុំនឹងជួយពិនិត្យមើលភាពខុសប្រក្រតី ឬសុវត្ថិភាពជូន!")
        else:
            # AI System Prompt
