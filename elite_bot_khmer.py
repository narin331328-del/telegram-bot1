...
def handle_photo(message):
  try:
    ...
    # #NOTE: កូដទាញយកអត្ថបទពី Cohere យ៉ាងមានសុវត្ថិភាព
    reply_text = ""
...
def handle_text_messages(message):
  try:
    ...
    # #NOTE: កន្លែងសម្រាប់កែសម្រួលអត្ថបទពេលអ្នកចុចលើប៊ូតុងម៉ឺនុយនីមួយៗ
    if user_input == "🛡️ Cybersecurity & IT Pro":
      ...
      # #NOTE: ទាញយកអត្ថបទពី Cohere Response ដោយសុវត្ថិភាពបំផុត
      reply_text = ""
      if hasattr(response, "message") and hasattr(response.message, "content"):
        reply_content = response.message.content
        if isinstance(reply_content, list):
    ...
    bot.send_message(
        ...
        reply_text
        if reply_text
        else "⚠️ សូមអភ័យទោស ប្រព័ន្ធមិនទាន់ទទួលបានទិន្នន័យឆ្លើយតបទេ។",
        reply_markup=get_main_menu(),
    )
  except Exception as e:
    bot.send_message(
...
if __name__ == "__main__":
  ...
  # #CRITICAL: skip_pending=True ຊួយជម្រះចោលសារចាស់ៗដែលបង្កឱ្យមាន Error កន្លងមក
